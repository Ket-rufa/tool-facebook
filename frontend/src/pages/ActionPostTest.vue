...
import ActionPostPanel from '../components/action/ActionPostPanel.vue'
import ActionLogList from '../components/action/ActionLogList.vue'
import { actionsService } from '../services/actionsService'
import { sessionService } from '../services/sessionService'
import type { ActionResponse, SessionStatus, ActionLog, CreatePostRequest } from '../types/action'
...
const handleTestPost = async (payloads: CreatePostRequest[]) => {
  isLoading.value = true
  latestResult.value = null
  
  try {
    for (const payload of payloads) {
      const res = await actionsService.createPost(payload)
...
<template>
  <div class="test-page">
    <div class="page-header">
      <h1 class="page-title">Thử nghiệm Đăng Bài (Post Status)</h1>
      <p class="page-desc">Giả lập và kiểm tra hành vi auto đăng bài viết lên Tường nhà (Timeline).</p>
    </div>
